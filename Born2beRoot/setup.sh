...
awk '{ if (NR == 25) print "password        requisite                       pam_pwquality.so retry=3 minlen=10 ucredit=-1 dcredit=-1 maxrepeat=3 reject_username difok=7 enforce_for_root"; else print $0}' /etc/pam.d/common-password > tmp
mv tmp /etc/pam.d/common-password
